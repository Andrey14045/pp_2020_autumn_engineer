...
//    MPI_Scatter(src.data(), length, MPI_INT, local_vec.data(), length, MPI_INT, 0, MPI_COMM_WORLD);
//    if (mpi_rank == 0 && rem != 0) {
...
//                MPI_Recv(tmp_vec.data(), num_op_length, MPI_INT, part, 1, MPI_COMM_WORLD, &status1);
//                local_vec = Merge(local_vec, tmp_vec);
//                loc_size += num_op_length;
//            }
//            if (mpi_rank == pow2(num_op - 1) * (i - 1)) {
//                MPI_Send(local_vec.data(), num_op_length, MPI_INT, 0, 1, MPI_COMM_WORLD);
//                return local_vec;
//            }
//        }
...
//            std::vector<int> tmp_vec(num_op_length);
//            MPI_Status status3;
//            MPI_Recv(tmp_vec.data(), num_op_length, MPI_INT, part, 3, MPI_COMM_WORLD, &status3);
//            local_vec = Merge(local_vec, tmp_vec);
//            loc_size += num_op_length;
//        }
//        if (mpi_rank % pow2(num_op) != 0) {
//            part = mpi_rank - pow2(num_op - 1);
//            MPI_Send(local_vec.data(), num_op_length, MPI_INT, part, 3, MPI_COMM_WORLD);
//            return local_vec;
//        }
//        num_op++;
//        i = i / 2;
